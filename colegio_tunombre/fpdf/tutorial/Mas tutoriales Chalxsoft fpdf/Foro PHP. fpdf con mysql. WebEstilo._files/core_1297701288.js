...
{out+=obj;}
return out;}},serialiseJSON:function(object,flds)
...
{var pass='';var name='';var next=null;if(isNaN(elem))
{name+=elem+':';}
...
{return this._testCssProperty("borderRadius");};for(var testName in tests)
{this.featureSupport[testName]=tests[testName].call(this);}},_css:function(t)
...
{$iTXT.metrics.Context.stopXfer=(new Date()).getTime();}},_corsData:function(opts)
...
{url+="&";}
url+="chunkkey="+opts.chunkKey;url+="&data=";var chunkSize=$iTXT.core.Ajax.maxURLLength-url.length;var totalChunks=Math.ceil(data.length/chunkSize);var chunksArr=[];for(var i=0;i<totalChunks;i++)
{if(chunkSize>data.length)
{chunkSize=data.length;}
var dataChunk=data.substring(0,chunkSize);if("%"==dataChunk.charAt(dataChunk.length-1))
{dataChunk=dataChunk.substring(0,dataChunk.length-1);data=data.substring(chunkSize-1);}
else if("%"==dataChunk.charAt(dataChunk.length-2))
{dataChunk=dataChunk.substring(0,dataChunk.length-2);data=data.substring(chunkSize-2);}
else
{if(i+1<totalChunks)
