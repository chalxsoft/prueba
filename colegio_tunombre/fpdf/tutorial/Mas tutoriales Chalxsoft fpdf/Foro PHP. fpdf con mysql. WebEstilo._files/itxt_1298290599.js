...
$iTXT.js.loader["$iTXT.itxt.Controller"]=true;$iTXT.itxt.Controller_Load=function(){var undefined;var $itxtUtil=$iTXT.core.Util;$iTXT.itxt.Controller=$iTXT.core.Class.create({templateLoadUnSubFunc:null,jsLoadUnSubFunc:null,initialiserLoaded:false,templatesLoaded:false,requiresContextualization:false,init:function(_options)
...
{if(!$iTXT.debug.CurrentConsole)
{window.setTimeout($iTXT.core.Event.bind(this,this.kickOff),100);return;}}
if(true==$iTXT.js.gaEnabled){new $iTXT.itxt.GoogleAnalytics({gaID:$iTXT.js.gaTrackingId});}else{}
...
{var initialiserUrl=$iTXT.js.serverUrl+$iTXT.cnst.CONTROLLER_INITIALISER+"?ts="+$itxtUtil.ts();this.contentLength=0;try
...
{initialiserUrl+="&refurl="+encodeURIComponent(document.location.href);}
...
{this.getRelatedContentDetails();$iTXT.fire("$iTXT:metrics:evt",{n:"contint",t:'interval'});var contextualizerUrl=$iTXT.js.serverUrl+$iTXT.cnst.CONTROLLER_CONTEXTUALIZER+"?ts="+$itxtUtil.ts();var extraPrms=["dfr","rc.a.d","rc.a.t","rc.a.st"];contextualizerUrl+=this._buildQueryParams($iTXT.glob.params,extraPrms);contextualizerUrl+="&pagecl="+this.contentLength;contextualizerUrl+='&jsoncl='+$iTXT.data.Context.tree.length;contextualizerUrl+='&ppc='+$iTXT.data.Context.getProcessedPercentage();contextualizerUrl+="&hn="+$iTXT.data.Context.textNodes.length;var opts={url:contextualizerUrl,callback:$iTXT.core.Event.bind(this,this.contextualizerLoad),data:$iTXT.data.Context.tree}
$iTXT.fire("$iTXT:contextualizer:drop",this);$iTXT.core.Ajax.postData(opts);}},contextualizerLoad:function()
...
{$iTXT.fire("$iTXT:metrics:evt",{n:"advint",t:'interval'});var advertiserUrl=$iTXT.js.serverUrl+$iTXT.cnst.CONTROLLER_ADVERTISER+"?ts="+$itxtUtil.ts();advertiserUrl+=this._buildQueryParams($iTXT.glob.params);$iTXT.fire("$iTXT:advertiser:drop",this);$itxtUtil.dropScript(advertiserUrl,$iTXT.core.Event.bind(this,this.advertiserLoad));}},advertiserLoad:function()
...
{this.dropSS(script);}}}},dropSS:function(s)
...
{var src=('https:'==document.location.protocol?'https://ssl':'http://www')+'.google-analytics.com/ga.js';$iTXT.core.Util.dropScript(src);}
var t=this;_gaq.push
(function()
{t.ready();});$iTXT.core.$(document).itxtBatchSubscribe([["$iTXT:tt:open",$iTXT.core.Event.bind(this,this._ttOpn)],["$iTXT:tt:click",$iTXT.core.Event.bind(this,this._ttClicked)],["$iTXT:tt:mouse:over",$iTXT.core.Event.bind(this,this._ttHover)],["$iTXT:hooks:loaded",$iTXT.core.Event.bind(this,this._hookEvt)]]);},ready:function()
{this.gaReady=true;_gat._createTracker(this.gaID,this.trackerName);_gaq.push([this.trackerName+'._setDomainName','none']);_gaq.push([this.trackerName+'._setAllowLinker',true]);_gaq.push([this.trackerName+'._setAllowHash',false]);_gaq.push([this.trackerName+'._trackPageview']);for(var i=0;i<this.initialGAQueue.length;i++)
...
{var evtObj=[this.trackerName+"."+name];if($iTXT.core.Util.isArray(data))
{evtObj=evtObj.concat(data);}
else
{evtObj.push(data);}
_gaq.push(evtObj);}
else
{this.initialGAQueue.push([name,data]);}},_ttOpn:function(e)
