...
{var td=(new Date()).getTime()-this.ccTS;var tout=Math.max(250-td,0);window.setTimeout($iTXT.core.Event.bind(this,this._allLoaded),tout);},beforeOpen:function()
{if(null!=this.template)
...
{if(this.template)
{this.template.remove();this.template=null;}}});}
$iTXT.js.loader["$iTXT.ui.Hook"]=true;$iTXT.ui.Hook_Load=function(){var undefined;$iTXT.ui.HookManager={id:0,hookedCounter:0,textNodes:[],hooks:[],unHookedHooks:[],initSkip:0,maxKWNode:5,maxKWPara:5,keywordPadding:0,keywordInterval:0,placePerParagraph:0,paragraphCounts:{},nodeID:1,add:function(hook)
{if(hook)
...
{}
var nodesSinceHook=this.keywordInterval;this.textNodes.itxtEach(function(o)
{if(this.placePerParagraph)
{var pHookCount=0;var extraMask=0;$iTXT.core.$A(o).itxtEach(function(pn)
...
{var nc=this._searchNode(o,0,this.maxKWNode,0,nodesSinceHook).nc;if(nc>0)
{nodesSinceHook=1;}
else
{nodesSinceHook++;}}},this);}
else
...
{var hk=this.unHookedHooks[i];var found=false;if(nodeHookCount<maxHooks)
...
{newNodes.push(document.createTextNode(followingText));var pNode=n.parentNode||document.body;for(var i=0;i<newNodes.length;i++)
...
if(!this.mouseOver)
{this.mouseOverTS=(new Date()).getTime();var doc=document.documentElement;this.mouseOverPos={x:e.clientX+doc.scrollLeft,y:e.clientY+doc.scrollTop};var t=this;this.delayedOverTO=setTimeout(function(){t.fireMouseOver(target);},10);}},fireMouseOver:function(target)
...
{t.hideTooltip();$iTXT.core.$(document).itxtFire("$iTXT:tt:after:minimise");}};$iTXT.core.$(document).itxtFire("$iTXT:tt:before:minimise");new $iTXT.fx.Combination(combOpts);}
else
...
this.hideTID=window.setTimeout(function(){$iTXT.core.$(document).itxtFire("$iTXT:tt:close",{closeSource:$iTXT.data.TTCloseSource.MOUSEOUT});},(t||hideTO));}},_hookIn:function(e)
{if(this.hideTID!=-1&&e.data&&e.data.hook&&e.data.hook==this.currentHook)
{window.clearTimeout(this.hideTID);this.hideTID=-1;}},_hookOver:function(e)
{if(this.hideTID!=-1&&e.data&&e.data.hook&&e.data.hook==this.currentHook)
{window.clearTimeout(this.hideTID);this.hideTID=-1;}
...
{var t=this;var adParams=e.data.hook.ad.params;var tooltipDelay=adParams.getInt("tt.open.delay",adParams.getInt("ttd",100));if(tooltipDelay>1000)
{tooltipDelay=750;}
this.openTID=window.setTimeout(function(){t._showOnHook(e.data.hook,e.data.bounds);},tooltipDelay);}},_hookOut:function(e)
...
{window.clearTimeout(this.openTID);}
else
{}}
...
{try
{if(this.isOpen)
{$iTXT.core.$(document).itxtFire("$iTXT:tt:close",{closeSource:$iTXT.data.TTCloseSource.OVERNEWHOOK,tso:this.getTimeSinceOpen()});}
this.showTail();this.currentHook=hk;this.currentAdvert=hk.ad;this.currentHookBB=bb;this.currentHook.setState(true);if(undefined!=hk.ad)
...
{throw(e);}},openWithAdvert:function(advert,leftPos,topPos,posState)
...
{adHvBgColor="#f6f6f6";}
this.options.hbgcol=$iTXT.core.Util.validHexColor(tmplopts.ttbghcol||adHvBgColor);this.options.tthd=adopts.getInt("tthd",this.defaultOptions.tthd);this.options.mintthd=adopts.get("mintthd",0);this.chrome.setAdvert(ad);this.shadow.setAdvert(ad);if(this.options.invisible)
...
{var dX=0,dY=0,sW=this.content.getWidth(),sH=this.content.getHeight();var newSize=this.content.tryResize(null,null);if(isNaN(sW))
{sW=newSize[0];}
if(isNaN(sH))
{sH=newSize[1];}
...
this.header=new tmpl.options.customHeader({ps:this.options.ps,bgcol:this.options.bgcol,draggable:this.options.draggable});this.addChild(this.header);this.defaultHeader.hide();}
...
{$iTXT.core.$(document).itxtFire("$iTXT:tt:before:position");if(opts.bb&&opts.tt)
{var ttW=opts.tt.width;var ttH=opts.tt.height;var bbL=opts.bb.left;var bbT=opts.bb.top;var bbR=opts.bb.left+opts.bb.width;var bbB=opts.bb.top+opts.bb.height;var ttps=$iTXT.ui.TooltipPosition;var posArr={};posArr[ttps.AR]={left:bbR-this.ttOffX,top:bbT-ttH-this.ttOffY,width:ttW,height:ttH,weight:0};posArr[ttps.AL]={left:bbL-ttW+this.ttOffX,top:bbT-ttH-this.ttOffY,width:ttW,height:ttH,weight:0};posArr[ttps.BR]={left:bbR-this.ttOffX,top:bbB+this.ttOffY,width:ttW,height:ttH,weight:0};posArr[ttps.BL]={left:bbL-ttW+this.ttOffX,top:bbB+this.ttOffY,width:ttW,height:ttH,weight:0};var sSize=$iTXT.core.Util.getWindowSize();var dScroll=$iTXT.core.Util.getPageScroll();var sb={left:dScroll[0],top:dScroll[1],width:sSize[0],height:sSize[1]};posArr[ttps.AR].weight-=100*(1-$iTXT.core.Math.intersectsPercentage(posArr[ttps.AR],sb));posArr[ttps.AL].weight-=100*(1-$iTXT.core.Math.intersectsPercentage(posArr[ttps.AL],sb));posArr[ttps.BR].weight-=100*(1-$iTXT.core.Math.intersectsPercentage(posArr[ttps.BR],sb));posArr[ttps.BL].weight-=100*(1-$iTXT.core.Math.intersectsPercentage(posArr[ttps.BL],sb));var artd=((posArr[ttps.AR].top-sb.top));if(artd<0)
{posArr[ttps.AR].weight+=this.TOPOFFSCREENWEIGHT*(artd/posArr[ttps.AR].height);}
var artdExp=((posArr[ttps.AR].top-sb.top)-posArr[ttps.AR].height);if(artdExp<0)
{posArr[ttps.AR].weight+=this.EXPTOPOFFSCREENWEIGHT*(artdExp/posArr[ttps.AR].height);}
var arrd=(((sb.left+sb.width)-(posArr[ttps.AR].left+posArr[ttps.AR].width)));if(arrd<0)
{posArr[ttps.AR].weight+=this.OFFSCREENWEIGHT*(arrd/posArr[ttps.AR].width);}
var arrdExp=(((sb.left+sb.width)-(posArr[ttps.AR].left+posArr[ttps.AR].width))-posArr[ttps.AR].width);if(arrdExp<0)
{posArr[ttps.AR].weight+=this.EXPOFFSCREENWEIGHT*(arrdExp/posArr[ttps.AR].width);}
var altd=((posArr[ttps.AL].top-sb.top));if(altd<0)
{posArr[ttps.AL].weight+=this.TOPOFFSCREENWEIGHT*(altd/posArr[ttps.AL].height);}
var altdExp=((posArr[ttps.AL].top-sb.top)-posArr[ttps.AL].height);if(altdExp<0)
{posArr[ttps.AL].weight+=this.EXPTOPOFFSCREENWEIGHT*(altdExp/posArr[ttps.AL].height);}
var alld=((posArr[ttps.AL].left-sb.left));if(alld<0)
{posArr[ttps.AL].weight+=this.OFFSCREENWEIGHT*(alld/posArr[ttps.AL].width);}
var alldExp=((posArr[ttps.AL].left-sb.left)-posArr[ttps.AL].width);if(alldExp<0)
{posArr[ttps.AL].weight+=this.EXPOFFSCREENWEIGHT*(alldExp/posArr[ttps.AL].width);}
var brbd=(((sb.top+sb.height)-(posArr[ttps.BR].top+posArr[ttps.BR].height)));if(brbd<0)
{posArr[ttps.BR].weight+=this.OFFSCREENWEIGHT*(brbd/posArr[ttps.BR].height);}
var brbdExp=(((sb.top+sb.height)-(posArr[ttps.BR].top+posArr[ttps.BR].height))-posArr[ttps.BR].height);if(brbdExp<0)
{posArr[ttps.BR].weight+=this.EXPOFFSCREENWEIGHT*(brbdExp/posArr[ttps.BR].height);}
var brrd=(((sb.left+sb.width)-(posArr[ttps.BR].left+posArr[ttps.BR].width)));if(brrd<0)
{posArr[ttps.BR].weight+=this.OFFSCREENWEIGHT*(brrd/posArr[ttps.BR].width);}
var brrdExp=(((sb.left+sb.width)-(posArr[ttps.BR].left+posArr[ttps.BR].width))-posArr[ttps.BR].width);if(brrdExp<0)
{posArr[ttps.BR].weight+=this.EXPOFFSCREENWEIGHT*(brrdExp/posArr[ttps.BR].width);}
var blbd=(((sb.top+sb.height)-(posArr[ttps.BL].top+posArr[ttps.BL].height)));if(blbd<0)
{posArr[ttps.BL].weight+=this.OFFSCREENWEIGHT*(blbd/posArr[ttps.BL].height);}
var blbdExp=(((sb.top+sb.height)-(posArr[ttps.BL].top+posArr[ttps.BL].height))-posArr[ttps.BL].height);if(blbdExp<0)
{posArr[ttps.BL].weight+=this.EXPOFFSCREENWEIGHT*(blbdExp/posArr[ttps.BL].height);}
var blrd=((posArr[ttps.BL].left-sb.left));if(blrd<0)
{posArr[ttps.BL].weight+=this.OFFSCREENWEIGHT*(blrd/posArr[ttps.BL].width);}
var blrdExp=((posArr[ttps.BL].left-sb.left)-posArr[ttps.BL].width);if(blrdExp<0)
{posArr[ttps.BL].weight+=this.EXPOFFSCREENWEIGHT*(blrdExp/posArr[ttps.BL].width);}
var scpos=$iTXT.core.Util.getPageScroll();var arTop=posArr[ttps.AR].top-sb.top+scpos[1];if(arTop<0)
{posArr[ttps.AR].weight-=500;}
var alTop=posArr[ttps.AL].top-sb.top+scpos[1];if(alTop<0)
{posArr[ttps.AL].weight-=500;}
var avList=[{w:10,n:"IFRAME"},{w:10,n:"OBJECT"},{w:10,n:"EMBED"}];for(var i=0;i<avList.length;i++)
{var avN=avList[i];var nLst=document.getElementsByTagName(avN.n);var nLstLen=nLst.length;for(var i2=0;i2<nLstLen;i2++)
{var nd=nLst[i2];var nbb=$iTXT.core.$(nd).itxtBounds();posArr[ttps.AR].weight-=avN.w*($iTXT.core.Math.intersectsPercentage(posArr[ttps.AR],nbb));posArr[ttps.AL].weight-=avN.w*($iTXT.core.Math.intersectsPercentage(posArr[ttps.AL],nbb));posArr[ttps.BR].weight-=avN.w*($iTXT.core.Math.intersectsPercentage(posArr[ttps.BR],nbb));posArr[ttps.BL].weight-=avN.w*($iTXT.core.Math.intersectsPercentage(posArr[ttps.BL],nbb));}}
var posState=ttps.AR;if(posArr[ttps.AL].weight>posArr[posState].weight)
{posState=ttps.AL;}
...
{posState=dbgTTPS;}}
var returnOptions={left:posArr[posState].left,top:posArr[posState].top,state:posState};return returnOptions;}}}}
...
this.closeBtn.alt=this.closeBtn.title=this.options.clsTxt;this.closeBtn.itxtChangeSrc(this.options.clsSrc);this.whatBtn.alt=this.whatBtn.title=this.options.whtTxt;this.whatBtn.itxtChangeSrc(this.options.whtSrc);if(""==this.options.clsSrc)
{this.closeBtn.itxtHide();}
else
{this.closeBtn.itxtShow();}
if(""==this.options.whtSrc)
